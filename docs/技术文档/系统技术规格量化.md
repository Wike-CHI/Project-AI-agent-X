...
- **目标用户**：10-100 用户规模
- **响应延迟**：2-5 秒
- **LLM策略**：云端 MiniMax API + Kimi API

### 1.1 LLM 配置矩阵

| 场景 | 模型 | 显存需求 | 响应速度 | 适用场景 |
|-----|------|---------|---------|---------|
| **云端主力** | MiniMax M2.1 | - | 中 | 复杂推理、知识查询 |
| **云端备选** | Kimi K2.5 | - | 中 | 备选、专项能力 |
| **备用** | OpenAI GPT-4o-mini | - | 中 | API网关降级 |
...
class LLMGateway:
    ...
    async def generate(
        ...
        # 尝试首选模型
        model = preferred_model or "minimax-m2.1"
...
# LLM 配置
LLM_PROVIDER=minimax  # minimax, kimi, openai
